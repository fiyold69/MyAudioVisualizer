...
void fileSelected(File selection)
{
    if (selection == null) {
        ...
        fftSize = 1024;
        player = minim.loadFile( selection.getAbsolutePath(), fftSize);
...
void draw()
{
    ...
    float bassRawEnergy = fft.calcAvg(20, 100);
    float offset = 0.5; //adjust the SWELL of bass
    ...
    for (int y = 0; y < rows; y++) {
        ...
        for (int x = 0; x < cols; x++) {
            ...
            float factor = 0.05; // the Pixelation (Roughness of Grid)
